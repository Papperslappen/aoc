    ...
    while let Some((remain, numbers)) = stack.pop() {
        ...
        if remain > 0 {
            if let Some(next) = numbers.first() {
                ...
                if remainstring.ends_with(&next.to_string())
                    && remainstring.len() > next.to_string().len()
                {
                    let new_number = remainstring
                        .strip_suffix(&next.to_string())
                        .unwrap()
                        .parse()
                        .unwrap();
...
pub(crate) fn solution(read_input: Vec<String>) -> (i64, i64) {
    let result_a = read_input
        ...
        .filter(|(target, numbers)| check_add_mul(*target, numbers))
        .map(|(target, _)| target)
        .sum();
    ...
    let result_b = read_input
        ...
        .filter(|(target, numbers)| check_add_mul_concatenate(*target, numbers))
        .map(|(target, _)| target)
        .sum::<i64>();
    (result_a, result_b)
}
